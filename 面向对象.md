...
static void Main(string[] args)
{
    ...
    Console.WriteLine(Path.GetFileName(str));//获取文件名以及拓展名
    ...
    Console.WriteLine(Path.GetFullPath(str));//获取文件全路径，绝对地址
    Console.WriteLine(Path.Combine(@"c\abx","ex.txt"));//连接两个字符串作为路径
}
```
...
| 方法                  | 作用             | 输入 → 输出         |
| :-------------------- | :--------------- | :------------------ |
| **SerializeObject**   | 对象转JSON，打包 | C#对象 → JSON字符串 |
| **DeserializeObject** | JSON转对象，拆包 | JSON字符串 → C#对象 |

1. **反序列化（JSON → C# 对象）**
   - **默认大小写不敏感**：Newtonsoft.Json 在反序列化时**自动忽略属性名大小写**，能智能匹配 JSON 的 `camelCase`（如 `"name"`）和 C# 的 `PascalCase`（如 `Name`）。
     - 例如：JSON 中的 `{"name":"Alice"}` 可直接映射到 C# 类的 `public string Name { get; set; }`，无需额外配置 (#)。
   - **原理**：此设计旨在提升兼容性。
2. **序列化（C# 对象 → JSON）**
   - **默认输出 PascalCase**：C# 属性名 `UserName` 序列化为 `"UserName"`，而非 `"userName"` 。
   - **需显式配置**才按照 `camelCase`命名法能输出。
