...
注意：值类型会进行装箱，若为`Int`类型，因为内部重写了`Equals`方法，会比较值(结构体较复杂，此处不讨论)
...
:two:`bool Equals()`,用于判断两个对象是否相等，默认与`==`一致。
...
为了避免装箱与拆箱可使用`IEquatable`接口来进行比较：
...
Encoding e = Encoding.GetEncoding(936);//通过代码页获取(编码的身份证)
Encoding en = Encoding.GetEncoding("UTF-8");//通过名称获取
```
