...
默认情况下，每个类实例都会保存一份成员的副本，这些成员统称为实例成员；而有的成员被类的所有实例共享，统称为静态成员，在`Loader Heap`存储，且只有一份拷贝。换句话说，实例成员与实例绑定，而静态成员与类型绑定。

值类型静态字段存储在Loader Heap中(其访问速度快)，引用类型静态字段的引用存储在Loader Heap中，实际数据存储在托管堆中。

生命周期：静态成员从访问该类型时完全加载，直到程序结束销毁静态成员；对于引用类型的成员，想提前销毁，可通过设置为null,切断与托管堆中的对象的引用，这样在GC时，就会销毁该对象实际数据。

实例成员实例成员在实例化后才会分配初始化内存，直到对象被GC回收结束。
...
创建类的实例时，字段都会有清零操作（值由类型决定,数值类型为`0`，引用类型为`null`，布尔值为`false`），若存在初始值，后续通过字段初始化器给字段赋值（也就是显示赋值）。

```c#
public class Example
{
    private int a = 10;    // 清零后设为10
    private string b = "text"; // 清零后设为"text"
}
```
...
1. 实例化一个`car`对象，字段清零→字段初始化器执行（有的话）。
...
1. 自动属性访问器后可赋一个默认值，用以显示初始化字段(自动属性存在自动属性初始化器，与字段初始化器同级别，执行顺序与代码位置相关)。
2. 自动属性必须要有`get`访问器。
...
3. **实例字段/自动属性初始化**：在创建类的实例时，实例字段或自动属性的后背字段会进行初始化。
