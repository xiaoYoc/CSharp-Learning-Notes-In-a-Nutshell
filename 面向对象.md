...
public class Program
{
    ...
    {
        Person p1;//仅栈上开辟空间，有未定义值，堆中未开辟空间
        Person p2 = null;//栈中空引用，堆中未开辟空间
...
2. 实例成员在实例化后才会分配堆内存，直到对象被GC回收结束。
...
| 生命周期   | 类型首次实例化或首次访问该类型成员→应用卸载 | 引用：类型首次实例化或首次访问该类型成员→应用卸载 ，指向的对象：可能短于静态值类型字段 (可能被GC 回收) |
...
> 成员常量可以看作是**值不可更改、编译时确定(须是字面量)、无需内存存储**的特殊静态值,所以声明时必须初始化.
...
2. 实例方法中，既可以使用静态成员(对所有对象共享)，也可以访问实例成员。
...
:one: ​创建一个`Person`类，其中有一个`Friend[]`类型的成员`friends`，通过`Add`方法向其中添加新朋友。

```c#
 public class Person
 {
     public readonly string name = "lzb";
...
public class Person 
{
   ...
   private Person()
    ...
    //外部只读，不能修改
    //静态成员之所以可以初始化相同类型，正是因为它是"一次性"的类级别初始化，不会产生无限递归的问题。
...
:two:`readonly`字段声明时未初始化，初始化需在对应的构造函数中完成。
...
internal  static class Test
{
    ...
    public readonly static double radius;//只能放在静态构造函数中进行初始化
}
```
...
调试时属性的值可能与预设想的不一致，是因为查看属性时调试器隐式调用了`get`方法，执行了其中的逻辑，所以在调试时应注意到此问题，尽可能去查看字段，而不是属性。
...
> 3. 不能同时为两个访问器设置访问权限(保证属性的访问权限有意义 )

### 只读属性

只读属性通常用一个`get`访问器实现 或者将`set`访问器私有化来实现，可在构造函数中初始化字段值。

:red_circle:私有化访问器，外部无法访问，但类内部可以访问。
...
只读属性要求不能修改其引用，但能修改内部成员，与`readonly只读字段`类似。
...
需要注意:red_circle: 自动只读属性可以在构造函数中赋值，与普通的只读属性相比。
...
public static void Main(string[] args)
{
    ...
    tractor.Name = "Test";//无法在外部为只读属性赋值
    tractor.Show();
}
```
...
internal class Ticket
{
    public Ticket(double distance) 
    {
        ...
        this.Distance = distance;//自动只读属性可以在构造函数中赋值
    }
...
:red_circle:静态成员建议放在静态构造函数中初始化。对于`readonly`修饰的只读静态字段，强制放在静态构造中初始化。
...
public class Car 
{
    ...
    public string _type = Show("实例字段初始化器执行");
    public string Wheel { set; get; } = Show("实例属性初始化器执行");

    public static string _year = Show("静态字段初始化器");

    public static string Brand { set; get; } = Show("静态属性初始化器执行");
...
//静态字段初始化器,静态属性初始化器执行,静态构造执行
//实例字段初始化器执行,实例属性初始化器执行,实例构造函数执行
```

## 对象初始化器

对象初始化时可以由`new`关键字后跟构造函数及参数列表，同时也可以放置一组初始化语句,使用逗号分隔。

``` c#
static void Main()
{
    Point p = new Point(1, 4)//无参数括弧可省略
    {
        //初始化语句
        _angle = 12.0,//逗号间隔
        _x = 12//顶替了构造函数中的值
...
:red_circle:给父类声明扩展方法，子类也能调用(继承)。
...
> 让对象可以像数组一样通过索引访问类内部的成员，使程序看起来更直观,比较适用于类中有数组的情况。
>
...
public class Index
 {
     public string? _space;
      //索引器没有名称，用this指代
    //[]中可以是任何类型
     public string this[ModeSpace space]
     {
         //可以有set访问器
         get
         {
             switch (space)
             {
                 case ModeSpace.ModelSpaceId:
                     _space ="1";
                     break;
                 case ModeSpace.DrawSpaceId:
                     _space = "2";
                     break;
                 case ModeSpace.OtherSpaceId:
                     _space = "3";
                     break;
                 default:
                     _space = "";
...
> 内存由`CLR`管理以及释放，GC是CLR的**核心组件**之一。
...
```c#
GC是CLR的核心组件之一Person person = new Person();
person = null;
...
:one:内部类：在一个类中声明一个类，意在将类分割为不同的模块，使用时需要用包裹者点出自己(类似成员，所以使用`private`修饰符外部不能访问该类)
...
判断一个对象是否可以转换为指定类型（值类型，引用类型都可），如果是，返回`true`，否则返回`false`。
...
```c#
if (list is CheckBox checkBox)  // 这里同时做类型检查和变量声明
{
    // 直接使用 checkBox 变量，不需要再次转换
    MessageBox.Show(checkBox.Content?.ToString());
}
```

如果`list`为`null`则直接返回`false`.
...
//此处设计很合理.会查找实际类型中的Equals方法。
    return objA.Equals(objB);
}
```

先判断两个对象引用是否相等，再调用`Equals`方法

:one:未重写，则调用默认的`Equals`方法(比较引用)

:two:重写，则调用重写的`Equals`方法(值类型比较值，字符串比较字符)。
...
注意：值类型会进行装箱，但是由于内部重写了`Equals`方法，会比较值(结构体较复杂，此处不讨论)

:two:`public static bool ReferenceEquals(object objA, object objB)`,用来比较引用类型对象，值类型始终是`false`

```c#
public static bool ReferenceEquals(object objA, object objB)
{
    //返回比较引用后的bool值
    //注意字符串的驻留机制：相同字面量的字符串指向同一个地址
    return objA == objB;
}
```
...
:bookmark:根据类型创建对象可使用`Activator.CreateInstance`方法。

1. `public static object? CreateInstance(Type type)`;创建该类型的无参构造对象。
2. `public static object? CreateInstance(Type type, params object?[]? args);`创建该类型的有参构造对象。
...
:one:`string ToString()`：将当前对象转换为字符串，默认返回当前对象的全名(`namespace+类型全名`)

`Console.WriteLine()`方法内部会调用该方法。
...
:two:`bool Equals()`,用于判断两个对象是否相等。

与`==`运算符的区别：

1. `==`运算符按照该类型的操作符进行比较,值类型比较值，引用类型比较地址(默认)，字符串比较字符；
3. `Equals()`方法比较时会查找重写后方法进行比较。

可对`Equal()`方法进行重写，同时还要重写`==`运算符以及`GeyHashCode()`，确保逻辑一致。
...
:three:哈希码`int GetHashCode`:根据对象的组成计算的一个数字。对于两个相等的实例，哈希码应该相同；有大量的不同对象，哈希码应该尽可能不同。

默认实现：对于引用类型，是基于引用构建的。对于值类型它是基于对象存储的值计算的。
...
```c#
 public override int GetHashCode()
 {
     ...
     {
        int hash = 17;
     	hash = hash * 31 + name?.GetHashCode()??0;//内部实现
     	hash = hash * 31 + age;
     	return hash;
     }
 }
...
为了避免装箱与拆箱可使用`IEquatable<T>`接口来进行比较：
...
子类对象调用`override`方法时(后面不在重写)，执行的是子类中的方法。
...
public class Position
{
    public Position()
    {
        //X = 11; 因为X时可读可写的，是给派生类字段赋值
        Y = 20;//因为Y是只读的，是给基类字段赋值
        Console.WriteLine($"{X},{Y}");//6,0
    }
    public virtual int X { get; set; } = 5;//基类字段初始化
    public virtual int Y { get;}
}
public class Point : Position
{
    public override int X { get; set; } = 6;
    public override int Y { get; }
    public void Fun() => Console.WriteLine($"{base.X},{base.Y}");//5,20
}
```

对于基类的自动属性初始化器不论只读还是可读可写，都是初始化基类字段。但在构造函数中有区别，所以对于重写的虚属性放到派生类中进行初始化。
...
4. 在抽象类中不能为抽象属性设置初始值，因为抽象属性仅声明契约，属性无任何实现，强制派生类实现细节；如果可以赋值，那么就相当于实现了抽象属性，与抽象类相违背，所以抽象成员统一交给派生类实现。
5. 可在抽象类的构造函数中或普通方法中进行访问抽象属性，因为此时访问的是派生类实现的属性，:red_circle:但是不建议这么做
   ...
   ```
6. 抽象类中可以包含普通类中所有的成员，构造函数，索引器，虚方法等。
7. 抽象方法和派生类`override`方法返回类型和方法签名以及访问权限要求一致。
...
5. 接口不能继承于类，类能继承多个接口
6. 一个类既继承于多个接口，又继承于类，必须先继承于类（语法）
...
明确规定某个成员属于某个接口，解决接口与接口，接口类中方法重名问题,用`接口.成员`实现方法；显式实现的成员**无法通过类实例直接访问**，必须通过接口引用调用。
...
//其他
 ...
 public interface IExtension<T> : IExtension
 {
     //隐藏父类成员
     new T Value { get;}
 }

 public class Show : IExtension<int>
 {
     //属于IExtension<int>接口
     public int Value { get => throw new NotImplementedException(); }
     //属于IExtension接口
     object IExtension.Value { get => throw new NotImplementedException();}
 }
```

较复杂的迭代器模式便采用隐藏父类成员来实现的。
...
当给字符串重新赋值时，旧值并没有消失，而是在堆中开辟一块空间存储新值。
...
static void Main(string[] args)
{
    ...
    Console.WriteLine("请下一个学员");
    string classTwo = Console.ReadLine();
    if (classOne.Equals(classTwo, StringComparison.OrdinalIgnoreCase)
        //按照数值大小进行排序，不考虑语言文化差异，且忽略大小写进行比较
        //按照文化比较，就不是根据码点进行比较，在土耳其语环境下，"i" 与 "İ" (带点大写 I) 被认为相等
    {
        Console.WriteLine("相同");
    }
    else 
...
```csharp
//分隔原理：将分隔字符替换为|，若在最前端或最后端有|，在最前端或最后端存在空字符串元素；在||中间也存在空字符串元素
//若存在||，中间有""
string str = "a cs adb";
// ||cs||db
//即["","",cs,"",db]
...
```csharp
 string str = "天,今天天气真好";
 int num = str.IndexOf('天');//重载：可传入字符
...
> 计算机只认识0和1，为了让计算机能"看懂"我们的文字，就需要我们定义翻译规则-这就是字符集。
>
> 字符集像一本字典，规定哪种字符对应哪个数字，但是没有规定在计算机中如何存储；至于如何存储，这就是编码管辖的事情了。
>
> 计算机存储字符时，需要将其拆分为1个或多个字节（每个字节8位，二进制表示）进行存储；
>
> 无论什么文件——文本文件、图片、视频、可执行程序、CAD图纸……它们在磁盘上存储的**都是字节数组**。
>
> 字节即存储数值的最小单位，每个字节表示 0~255 的值
>
> 
...
> 前`Unicode`时代，编码和字符集可以认为是一体的；`unicode`是第一个有两种不同编码方法的字符集。

:one:`ASCII `的诞生

- 1960年代，美国制定了`ASCII`（美国信息交换标准代码），用7位二进制数（即0~127）对应英文字母、数字和常用符号。例如 `A` 对应十进制的65，存储时就是二进制 `01000001`。
- ASCII只解决了英语的需求，其他语言无法表示。

:two: 各国扩展：代码页（Code Page）的出现

- 为了让计算机处理本国文字，各个国家或厂商在ASCII的基础上扩展了编码，这些扩展方案就是**代码页**。
- 例如：
  - 简体中文：GB2312/GBK（代码页936）
  - 繁体中文：Big5（代码页950）
  - 日文：Shift JIS（代码页932）
- **代码页的本质**：一张`字符→字节/字节序列`的映射表。每个字符被分配一个固定的数字（通常用1~2个字节表示），存储时就直接存储字节的二进制形式。
- **存储方式**：比如用GB2312保存汉字“中”，它对应的字节是 `0xD6` 和 `0xD0`（0x表示16进制前缀），那么在文件中就写入二进制 `11010110 11010000`（即两个字节 `0xD6` 和 `0xD0`）。
- **乱码的根源**：不同代码页对同一个二进制数字的解释不同。一个在中文系统下保存的文件，放到日文系统里打开，日文系统会用自己的代码页去解读那些二进制，导致显示成完全不同的字符（乱码）。

:three:Unicode 字符集的诞生

- 为了解决多语言混用和乱码问题，Unicode联盟于1991年发布Unicode 1.0，旨在为全世界所有字符分配一个**唯一的数字编号**，称为**码点（Code Point）**。例如汉字“中”的码点是 `U+4E2D`（十六进制 `4E2D`，十进制20013）。
- Unicode 只定义字符与数字的对应关系，不规定这些数字在计算机中如何存储。
- 码点根据`UTF-8`和`UTF-16`编码的规则在计算机中存储为字节数组。

:four:代码页转换

`Unicode`出现后，为了兼容旧数据和与世界接轨，系统内部对于不同的代码页维护一张新的映射表(NLS文件)，将字节序列直接映射到**Unicode码点**。例如，在代码页936（GBK）中，字节 ` `0xD6  0xD0`对应于 Unicode 码点 `U+4E2D`（“中”字）。

我们进行代码页转化时，会先将字符对应的`字节值`转化为码点，然后输出为`Unicode`字符串。

:five:代码页的定义扩大

在 .NET 的世界里，`代码页`这个词已经被微软扩展成了**通过指定与代码页关联名称和ID来获取编码**,不管你用的是古老的非Unicode编码，还是现代的Unicode编码（如UTF-8、UTF-16），它都有一个对应的数字编号和名称。

`C#`中编码设置使用`Encoding`类

```c#
Encoding encoding = Encoding.UTF8;//通过静态字段获取编码
Encoding e = Encoding.GetEncoding(936);//通过代码页ID获取(编码的身份证)
Encoding en = Encoding.GetEncoding("UTF-8");//通过代码页名称获取
```
...
static void Main(string[] args)
{
    string str = @"D:\LZBapplication\cad\inst\Autodesk ReCap\cad.exe";
    
    Console.WriteLine(Path.GetFileName(str));//获取文件名(含拓展名)
    Console.WriteLine(Path.GetFileNameWithoutExtension(str));//获取文件名不含拓展名
    Console.WriteLine(Path.GetExtension(str));//获取拓展名 .exe
    Console.WriteLine(Path.GetDirectoryName(str));//获取文件所在文件夹全路径
}
```

### `File`

> `File`静态类读取文件时在方法内部会处置资源，不需要使用`using`语句额外管理非托管资源。
...
flowchart LR
    A[所有文件] --> B[文本文件<br>可读]
    A --> C[二进制文件<br>字节流，需专用程序]
    
    %% 文本文件分类
    B --> B1[.txt .md .log<br>文档日志类]
    B --> B2[.csv .json .xml<br>数据格式类]
    B --> B3[.html .css .js<br>网页脚本类]
    B --> B4[.py .java .cs<br>源代码类]
    B --> B5[.yml .ini .cfg<br>配置设置类]
    
    %% 二进制文件分类
    C --> C1[.exe .dll .so<br>可执行程序类]
    C --> C2[.jpg .mp3 .mp4<br>媒体文件类]
    C --> C3[.pdf .docx .xlsx<br>办公文档类]
    C --> C4[.zip .rar .7z<br>压缩文件类]
    C --> C5[.db .sqlite .iso<br>系统数据类]
    
    %% 样式设置
    style A fill:#f0f8ff,stroke:#333,stroke-width:2px
    style B fill:#e6f7ff,stroke:#0066cc
    style C fill:#ffe6e6,stroke:#cc0000
    style B1 fill:#d9f2e6
    style B2 fill:#e6f2ff
    style B3 fill:#fff0e6
    style B4 fill:#f9f2e6
    style B5 fill:#f0e6ff
    style C1 fill:#ffe6e6
    style C2 fill:#fff0e6
    style C3 fill:#e6ffe6
    style C4 fill:#e6f2ff
    style C5 fill:#f2e6ff
```



##### `File.Read/WriteAllText `

:red_circle: 一次性写入方法会自动创建新文件。如果文件已存在，会被**完全覆盖**（不是追加）

| 方法名称                                             | 描述                                                         |
| :--------------------------------------------------- | :----------------------------------------------------------- |
| `public static string ReadAllText(String)`           | 打开一个文本文件，读取文件中的所有文本，然后关闭文件。       |
| `public static string ReadAllText(String, Encoding)` | 打开一个文件，使用指定的编码读取文件中的所有文本，然后关闭文件。 |
| `WriteAllText(String, String)`                       | 创建一个新文件，将指定的字符串写入文件，然后关闭文件。       |
| `WriteAllText(String, String, Encoding)`             | 创建一个新文件，使用指定的编码将指定的字符串写入文件，然后关闭文件 |

```c#
string data = File.ReadAllText("C:\\Users\\Administrator\\Desktop\\呆滞.json", 
                               Encoding.GetEncoding("UTF-8"));
Console.WriteLine(data);
```

##### `File.Read/WriteAllLines`

用于**一次性读取文本文件的所有行**，返回一个字符串数组，每个元素对应文件中的一行。

| 方法名称                                    | 中文描述                                                     | 返回值类型 |
| :------------------------------------------ | :----------------------------------------------------------- | :--------- |
| `ReadAllLines(String)`                      | 打开文本文件，读取文件的所有行，然后关闭文件。使用默认编码。 | `string[]` |
| `ReadAllLines(String, Encoding)`            | 打开文件，使用指定编码读取文件的所有行，然后关闭文件。       | `string[]` |
| `WriteAllLines(String, String[], Encoding)` | 创建一个新文件，使用指定编码将字符串数组写入文件，然后关闭文件。 | void       |
| `WriteAllLines(String, String[])`           | 创建一个新文件，将字符串数组写入文件，然后关闭文件。         | void       |

```c#
string[] s = File.ReadAllLines("C:\\Users\\Administrator\\Desktop\\12.txt");
//文件不存在则创建，存在则替换其中的文本
...
```c#
//打开一个二进制文件，将内容存储到字节数组，并关闭文件
byte[] buffer = File.ReadAllBytes(Path.GetFullPath("C:\\Users\\Administrator\\Desktop\\新建文本文档.txt"));
//Encoding是字符编码类，该方法指示使用UTF8编码来解析字节数组
...
static void Main()
{
    ...
    File.WriteAllBytes("C:\\Users\\Administrator\\Desktop\\新建文本文档.txt", bytes);
}
```
...
> 内部是以字符数组存储的，但是与`string`内部存储方式不同，较复杂。
...
| `public StringBuilder Remove(int startIndex, int length);`   | 从指定位置删除指定长度的字符串 |
...
> :one: 数据持久化是将内存中的数据模型转换为存储模型，以及将存储模型转换为内存中的数据模型。
>
> :two:`JSON`全称**JavaScript 对象简谱（JavaScript Object Notation）**国际通用的一种轻量级的数据交换格式。主要在网络通讯中用于传输数据，或本地数据存储和读取，易于人阅读和编写，同时也易于机器解析和生成，并有效地提升网络传输效率。

### `JSON`的配置规则

使用[sublimetext](https://text.sublime.net.cn/)编辑器创建`Json`文件。

:one:`JSON` 语法符号

| 符号        | 含义说明                 | 示例                            |
| :---------- | :----------------------- | :------------------------------ |
| `{}` 大括号 | 表示一个 JSON 对象       | `{"name": "John", "age": 30}`   |
| `[]` 中括号 | 表示一个 JSON 数组       | `["apple", "banana", "orange"]` |
| `:` 冒号    | 分隔键值对的键和值       | `"name": "John"`                |
| `,` 逗号    | 分隔多个键值对或数组元素 | `"name": "John", "age": 30`     |
| `""` 双引号 | 包裹键名和字符串值       | `"name": "John"`                |

:two: `JSON `值类型

| 值类型 | 示例               | 说明                       |
| :----- | :----------------- | :------------------------- |
| 数字   | `42`, `3.14`       | 整数或浮点数，不需要引号   |
| 字符串 | `"hello"`          | 文本内容，必须用双引号包裹 |
| 布尔值 | `true`, `false`    | 逻辑值，不需要引号         |
| 数组   | `[1, 2, 3]`        | 值的有序集合，用中括号包裹 |
| 对象   | `{"key": "value"}` | 键值对的集合，用大括号包裹 |
| null   | `null`             | 空值，表示没有值           |

我们使用[sublimetext](https://text.sublime.net.cn/)编辑器创建`Json`文件，并将下面的类转换为`Json`格式。
...
//大括弧代表一个对象
{
	...
	"ids" :[1,2,3,4],
	"students" : [
		{"name":"lLi","age":12,"sex":true},
		{"name":"lady","age":24,"sex":true}],
	"home" :{"address":"river","street":"东进道"},
	"son":null
}
```
...
![image-20250905000009759](assets/image-20250905000009759.png)

在[bejson](https://www.bejson.com/)中进行转换。

### `Newtonsoft.Json`

> `Newtonsoft.Json`类库中有个重要的类`JsonConvert`，用于序列化与反序列化。

核心方法汇总

| 方法                  | 作用       | 输入 → 输出         |
| :-------------------- | :--------- | :------------------ |
| **SerializeObject**   | 对象转JSON | C#对象 → JSON字符串 |
| **DeserializeObject** | JSON转对象 | JSON字符串 → C#对象 |
|                       |            |                     |

```c#
  public class Res
  {
      public Res(int id,string name,int number,string unit)
      {
          Id = id;
          Name = name;
          Number = number;
  ...
  public class Program
  {
      ...
      static void Main()
      {
          List<Res> list = new List<Res>()
          {
              new Res(1,"卫生管",22,"m"),
              new Res(2,"活接",1,"pcs"),
              new Res(3,"卡箍组件",23,"pcs"),
              new Res(4,"弯头",23,"pcs"),
          };
          //序列化，对象转化为JSON字符串
          string s = JsonConvert.SerializeObject(list,Formatting.Indented);//格式化为有条理的结构
          Console.WriteLine(s);
          //反序列化，JSON字符串转化为Csharp对象
         List<Res>? r = JsonConvert.DeserializeObject<List<Res>>(s);
      }
...
- **默认大小写不敏感**：Newtonsoft.Json 在反序列化时**自动忽略属性名或字段名大小写**，能智能匹配 JSON 的键名与属性名(或字段名)。
  - 例如：JSON 中的 `{"name":"Alice"}` 可直接映射到 C# 类的 `public string nAme { get; set; }`。
- **原理**：此设计旨在提升兼容性。
- 没有匹配到的字段或属性会有与类型相关的默认值。



### `FreeSpire.XLS`

> `FreeSpire.XLS`是一款轻量级的 Excel 处理库,我们可以使用代码方式将`excel`表格转化为`json`.

```c#
static void Main()
{
    string path = "C:\\Users\\Administrator\\Desktop\\plm.xlsx";
    Workbook workbook = new Workbook();//创建工作簿对象
    workbook.LoadFromFile(path);//指定路径，加载excel文件
    Worksheet worksheet = workbook.Worksheets[0];//第一个表

    DataTable dataTable = worksheet.ExportDataTable();
    //导出为dataTable对象，即内存中的一个虚拟表格
    JsonSerializerSettings settings = new JsonSerializerSettings()
    {
        Formatting = Formatting.Indented,//格式化
        //NullValueHandling = NullValueHandling.Ignore 忽略空值
    };
    var value = JsonConvert.SerializeObject(dataTable, settings);
    //转化为json对象，默认转化为数组，每一行对应一个对象，属性为该列的表头
    var lists = JsonConvert.DeserializeObject<List<Data>>(value);
    Console.WriteLine("成功转化为列表");
  //处理料号，如果旧品号不为空，则名称为 名称-旧品号形式
    var values = from item in lists
                 let NewName = item.OldId != null ? item.Name + "-" + item.OldId : item.Name
                 select new { item.Id, Name = NewName, item.Specifications,item.Unit,item.Company};
    var s = JsonConvert.SerializeObject(values, settings);
    File.WriteAllText("C:\\Users\\Administrator\\Desktop\\plmnew.json", s);
