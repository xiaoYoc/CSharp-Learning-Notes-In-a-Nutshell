...
静态成员生命周期：:one: 一旦创建，应用结束才会结束;:two:充当全局变量全局:three:效率高。
...
字段可以有初始值，若没有创建对象时编译器会自动给字段赋予初值（初值由类型决定,数值类型为`0`，引用类型为`null`，布尔值为`false`）。
...
:red_circle:`new`运算符作用：为指定类型的实例分配内存(换句话说类是不占内存的，而创建的对象是占内存的)。
...
字段类型可以是类型本身，但是要注意不能直接初始化：

```c#
public class Person 
{
    private Person p;//对象实例化后字段值为null
    private Person pp = new Person();//会造成内存溢出
}
```
...
:two:`readonly`字段声明时未初始化，初始化必在各自的构造函数中完成。
...
:one:静态构造函数通常用来初始化类的静态字段；静态构造函数不得有修饰符，有`static`关键字，不得带参数。

:two:不能手动调用，系统自动调用，且类中只能有一个静态构造函数
...
程序结束的时候，析构函数才会被执行，意在帮助我们释放资源(`GC`垃圾回收器调用)。
...
适用于多人开发时，同时存在同一个类，在类前加`partial`关键字，则代表一个完整的类被分给为几块，各个部分可以访问相关成员，本质就是一个整体。结构，接口，类都可以使用分部形式
...
针对所有类型，如果转换成功，返回一个`true`，否则返回一个`false`。
