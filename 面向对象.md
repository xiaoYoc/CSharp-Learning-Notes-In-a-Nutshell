...
>  把重复的成员单独封装到一个类中来作为父类，子类便可继承父类所有成员。对于父类的私有成员，子类无法直接访问，但可以通过父类暴露的方法间接访问。
>
...
public class Person 
{
    private int id;//父类中的id
    public string name;
}
public class Solider:Person 
{

    public int id;//子类中的id
    public string name;//隐藏父类成员
}
```

字段`id`与`name`内存中分别有两个，一个是父类的，另一个是子类的。

:red_circle: 不能在子类构造函数中为父类的自动只读属性赋值，因为父类已经实现了逻辑，不希望子类破坏，除非隐藏或重写。
...
当派生类隐藏父类成员时，可以使用`base.成员名`调用父类的成员。
...
判断一个对象是否是指定类对象，如果是，返回`true`，否则返回`false`。
...
将一个对象转化为指定对象，成功则返回对应的对象，否则返回`Null`。
