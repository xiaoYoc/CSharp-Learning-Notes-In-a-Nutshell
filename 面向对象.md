...
先判断两个对象引用是否相等，再调用重写的`Equals`方法，如果没有返回比较引用后的bool值。
...
> 编码就是 **文字 ↔ 数字** 的翻译规则。计算机只认识0和1，为了让计算机能"看懂"我们的文字，就需要我们定义翻译规则——这就是编码格式。由于早期编码混乱，同一串数字，在不同规则下表示不同文字，所以需要一套统一规则。
...
1. 现代应用一律用UTF-8
2. 处理中文旧文件才考虑GBK(简体与繁体)
3. 永远不要依赖系统默认编码

在`windows`中编码由代码页实现，类似一个表格，存放字符与数字的映射关系，也就是说通过读取代码页转化为编码对象

`C#`中编码属于`Encoding`类

```c#
Encoding encoding = Encoding.UTF8;//通过静态字段获取编码
Encoding e = Encoding.GetEncoding(936);//通过代码页编码获取(编码的身份证)
