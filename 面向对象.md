...
![image-20250529212337082](assets/image-20250529212337082.png)

## 实例成员与静态成员

默认情况下，每个类实例都会保存一份成员的副本，这些成员统称为实例成员；而有的成员被类的所有实例共享，统称为静态成员，在静态存储区域存储，且只有一份拷贝。

静态成员在类型首次调用前加载，即类未实例化前就可以直接访问，而实例成员在实例化后才会分配初始化内存。
...
静态成员特性：:one: 一旦分配内存，直到应用程序结束才会释放内存;:two:充当全局变量全局:three:效率高。

用途：用户的登录信息(`QQ`的各个界面上显示的用户信息)，系统配置信息等。

缺点：使用频率高的成员可作为静态类，否则会占用内存。

## 字段

### 实例字段

类的字段可以有初始值，若没有在创建对象时编译器会自动给字段赋予初值（初值由类型决定,数值类型为`0`，引用类型为`null`，布尔值为`false`）。

```c#
Console.WriteLine(default(int));
//可是用default(变量类型)查看默认值
```
...
字段类型可以是类型本身，但是要注意不能在内部进行初始化，否则会造成内存溢出：
...
2. 对象的初始化，给属性赋值会执行`set`方法，内部给字段赋值。
...
不需要显示声明一个私有变量，编译器会自动生成一个私有后备字段存储值（必须有`get`方法，否则类内部也无法获取），适合不需要复杂逻辑处理的属性。
...
:four:用于静态类中的拓展方法，像实例一样去调用方法，也可以用类名.方法去调用。
...
> 让对象可以像数组一样通过索引访问 其中的元素，使程序看起来更直观,比较适用于类中有数组的情况。
>
...
当引用类型的堆内存被回收时，析构函数才会执行，意在帮助我们释放资源(`GC`垃圾回收器主动调用析构函数)。
...
垃圾回收`Garbage Collector`简称`GC`,垃圾回收的过程是在遍历堆`Heap`上动态分配的所有对象，同过识别他们是否被引用来确定哪些对象是垃圾，哪些对象仍要继续使用。
...
1. `GC`只负责堆内存的垃圾回收，所以引用类型内存空间的分配与释放都是通过垃圾回收机制来管理的

2. 栈`stack`上的内存是由系统自动管理的，会自动分配和释放内存。
...
字段`id`与`name`内存中分别有两个，一个是父类的，另一个是子类的，子类对象调用同名成员优先调用自己的成员。

![image-20250604180956493](assets/image-20250604180956493.png)

:red_circle: 不能在子类构造函数中为父类的自动只读属性赋值，因为父类的成员已经初始化完毕，子类只能读取，除非隐藏或重写(此时的属性成员便是新成员，与基类无关)。
...
子类中的对象有两个`Name`属性，一个是父类的`Name`,一个是自己的`Name`；方法调用时根据所在的上下文读取成员数据。
...
1. 子类可以转换为父类,此时的对象只能访问基类中的成员，除非实现了多态：
   ...
   :one:子类方法override重写父类虚方法，调用子类的方法。
...
让不同子类有不同表现状态且同一子类状态唯一，可用`base`保留父类的行为。实现多态方式：虚方法，抽象类，接口。
...
1. 基类中属性有`set`和`get`访问器，如果是普通属性重写时可只覆盖基类中的一个访问器。

2. 若是自动属性必须覆盖基类中所有的访问器(因为派生类也会隐式生成一个独立的后背字段，若不重写`set`访问器，赋值时会给基类的后备字段赋值，自己本身的属性永远是系统自动初始化值，这种属性无意义)
...
public class Labrador : Dog
{
    //相当于优先继承Dog类中的override方法，不会向上查找Animal中的虚方法
}
...
4. 在抽象类中不能为抽象属性赋初始值，抽象属性无后备字段，仅声明契约，强制派生类实现细节；可在抽象类的构造函数中赋值，因为访问的是实际对象的属性。
...
7. 接口的子类必须实现该接口所有的方法，包括属性的访问器。
8. 实现接口的函数可以加`virtual`，让子类重写

:red_circle:注意定义在接口中属性称之为接口属性，而不是自动属性，纯接口定义，不会生成后背字段。
...
:bookmark:接口继承于接口时不需要实现，待类继承于接口时取实现所有方法。
