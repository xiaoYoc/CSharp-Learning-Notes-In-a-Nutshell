...
二进制`1000`表示的十进制8，则四位二进制表示的最大二进制为`2的5次方-1` .
...
:red_circle:有符号指的是可存放正负数；无符号指无负数；位数指的是存放二进制数字长度；不能赋值小数。

:red_circle:有符号类型位数第一位表示正负性，如`int a = -1`，二进制为`1000 0000 0000 0000 0000 0000 0000 0001` 。
...
静态类型变量存储在静态存储区域，访问所在类时便被初始化，整个程序结束时才会销毁变量。

值类型直接存储在静态存储区域中，若是引用类型则存储对象的引用，该引用依旧指向堆中的实际数据。
...
局部变量的生命周期:从声明那一点开始，到块结束，会释放局部变量占用的内存空间，销毁局部变量
...
Main函数中声明的变量只能在Main函数中使用，自定义函数是无法访问到。我们可以在类中通过声明静态字段来模拟全局变量，该类中所有方法都可以访问到，只有在结束后才会销毁静态字段。
...
| 优先级 | 运算符类型 | 示例                                                         |
| :----- | :--------- | :----------------------------------------------------------- |
| 1      | 括号 `()`  | `(x + y)`,括弧`()`可以改变优先级，先计算括弧内的内容，多层括弧先计算内层括弧 |
| 2      | `++`,`--`  | `++x`, `x++`                                                 |
| 3      | `* / %`    | `x * y`,模`%`与`/`以及`*`优先级相同                          |
| 4      | `+ -`      | `x + y`                                                      |
| 5      | =          | `x = y`                                                      |
...
数据和函数的集合，隐式密封，用来表示数据关系的集合,属于值类型，在栈中开辟空间。
...
结构体中不存在析构函数，因为结构体时分配在栈空间的，是由系统进行管理。
