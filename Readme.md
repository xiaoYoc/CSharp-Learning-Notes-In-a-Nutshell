...
从声明那一点开始，到块结束，会释放局部变量占用的内存空间，销毁局部变量。
...
常量的类型可以是预定义的简单类型，`string`类型以及`null`引用，不能是自定义类型(枚举除外)，因其需要在运行期初始化才能确定。
...
:two:枚举成员名即是整数常量名称,只是在声明赋值枚举变量前，要求先声明`枚举类型`。
...
>
> `item`类型如果和`collection`类型相互兼容，内部会进行类型强转。
