...
委托_`Delegate`：是一种特殊的类，本质是函数指针的升级版，其对象存放函数的引用地址（可将委托理解为存放函数的容器），通过委托调用方法时，实际是通过存储的地址间接跳到目标方法。

委托用来存储，传递方法。

委托管理的方法参数(数量和类型)和返回值需要和委托类型相兼容，与参数名无关。
...
> 内层函数可以引用包含在它外层函数的变量，即使外层函数的执行已经终止。
>
> :red_circle:外层函数变量提供的值并非创建时的值，而是闭包字段存储的最终值。
...
:three::委托绑定:委托指向闭包类的方法，变量声明周期延长至闭包被GC回收
...
public Program()
 {
     for (int i = 0; i < 10; i++)
     { //每次迭代结束,index变量被提升为闭包类的字段，生命周期延长，下一次迭代生成字段index，封装匿名方法
         int index = i;
